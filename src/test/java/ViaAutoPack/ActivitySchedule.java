...
public class ActivitySchedule extends BaseClass {
	public static Logger log = LogManager.getLogger(ActivitySchedule.class.getName());
	...
	public void activityScheduleTabTest(String un, String pw, String role) throws IOException {

		log.info("validateLandingPage Test Started");

		// create obbject and invom=ke methods
		...
		log.info("root node name 'Nayan Kumar' verified");
		hpo.nodeRR().click();
		hpo.groupTemplateTab().click();
		hpo.courseTyopGroup().click();
		log.info("Clicked Course type of Group Template");
		Assert.assertTrue(hpo.activitySchedulerTab().isDisplayed());
		log.info("Under Course type of Group Template ActivitySchedulerTab is found and verified");
	}
